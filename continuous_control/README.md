...
Random noises generated by Ornstein-Uhlenbeck process is added to actions to improve the exploration and make the training more efficient. Generate the noise array for 20 agents with a single OU process performed much better than applying 20 independent OU processes to 20 Reacher agents separately.

Leaky_relu() transformation function applied to Critic network speeded up the training process and got the goal (30+ average score) much faster than relu(). For example, the episodes first hitted the goal changed from 48 episodes (relu) to 33 episodes (leaky_relu). 
