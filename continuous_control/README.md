...
Random noises generated by Ornstein-Uhlenbeck process is added to actions to improve the exploration and make the training more efficient. It has a significant impact on the convergence.

Leaky_relu() activation function applied to Critic network speeded up the training process and got goal much faster than relu(). For example, the episodes first hitted the goal changed from 48 episodes (relu) to 33 episodes (leaky_relu). 
